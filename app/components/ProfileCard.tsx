import { ArrowRight, BadgeCheck, LayoutTemplate } from "lucide-react";
...
export default function ProfileCard({ profile, index = 0 }: ProfileCardProps) {
  const profilePic = profile.image || getProfileImage(profile.id);
  const profileHref = `/profiles/${profile.id}`;
  const editHref = `/profiles/${profile.id}/edit`;

  return (
    <div
      className="group bg-cream border border-graphite/10 rounded-sm overflow-hidden hover:shadow-lg hover:border-copper/30 transition-all duration-300 flex flex-col"
      style={{ animationDelay: `${index * 0.08}s` }}
    >
      <Link
        href={profileHref}
        className="block focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-copper/50"
      >
        {/* Headshot */}
        ...
        <div className="p-5">
          ...
          <div className="flex flex-wrap gap-1.5 mb-4">
            {profile.category.slice(0, 2).map((cat) => (
              <span
                key={cat}
                className="text-[10px] text-copper/80 border border-copper/25 px-2 py-0.5 rounded-sm tracking-wide"
              >
                {cat}
              </span>
            ))}
      ...
      <div className="px-5 pb-5 pt-0 border-t border-graphite/5">
        <Link
          href={editHref}
          className="inline-flex items-center gap-2 text-graphite/55 hover:text-copper text-xs font-medium transition-colors"
        >
